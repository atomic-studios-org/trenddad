...
isLoading.value = true
  await $fetch("/api/sendreference", {body: {email: email.value}, method: "POST"})
...
<template>
  <div v-if="openPopup" class="fixed  top-0 left-0 right-0 z-40 h-screen flex items-center justify-center bg-black bg-opacity-50">
<div class="relative py-10 z-50 px-10 bg-white border border-groove border-gray-300">
  <span @click="openPopup = false" class="absolute top-2 right-2 cursor-pointer text-xl">X</span>
 <div class="flex flex-col">
  <label>Email:</label>
        ...
        <span class="text-red-500" v-if="isWrongEmail">Email is not correct</span>
         <span class="text-blue-500" v-if="isSendEmail">Please check your inbox</span>
        <button
        @click="sendReference"
        :disabled="isLoading"
  ...
  <div class="absolute z-20 gap-1 flex top-0 h-6 w-screen items-center justify-center bg-purple-400 text-xs text-white">
        <span>Get 10% off, to get the code</span><span class="underline cursor-pointer" @click="openPopup = true" >click here</span><span>!</span>
      </div>
  <NuxtLink href="/" class="absolute top-8 md:top-4 md:left-8 lg:left-16 lef-2 decoration-none ml-4"
      ><img class="md:h-16 h-10 w-auto" src="/trend-dad.png" alt ="" /></NuxtLink
    >
  <div
